// Flow field visualization — curl noise driven particles
...
// ─── Simple 3D noise (hash-based gradient noise) ────────────────────────────
function hash3(ix, iy, iz) {
  let n = ix * 374761393 + iy * 668265263 + iz * 1440670567;
...
function noise3D(x, y, z) {
  ...
  const g000 = grad3(hash3(ix, iy, iz), fx, fy, fz);
  const g100 = grad3(hash3(ix + 1, iy, iz), fx - 1, fy, fz);
  const g010 = grad3(hash3(ix, iy + 1, iz), fx, fy - 1, fz);
  const g110 = grad3(hash3(ix + 1, iy + 1, iz), fx - 1, fy - 1, fz);
  const g001 = grad3(hash3(ix, iy, iz + 1), fx, fy, fz - 1);
  const g101 = grad3(hash3(ix + 1, iy, iz + 1), fx - 1, fy, fz - 1);
  const g011 = grad3(hash3(ix, iy + 1, iz + 1), fx, fy - 1, fz - 1);
  const g111 = grad3(hash3(ix + 1, iy + 1, iz + 1), fx - 1, fy - 1, fz - 1);
...
// ─── Curl noise: divergence-free vector field from noise gradients ──────────
function curlNoise(x, y, z, t) {
  ...
  // We compute curl of a potential field (noise_a, noise_b, noise_c)
...
export function init(canvas, container) {
  ...
  for (let i = 0; i < NUM_PARTICLES; i++) {
    resetParticle(i);
    life[i] = Math.random() * maxLife[i]; // stagger initial lifetimes

    // Color based on initial position
    const hue = (px[i] + BOUNDS) / (BOUNDS * 2);
    const color = new THREE.Color().setHSL(0.5 + hue * 0.3, 0.7, 0.55);
  ...
  function animate() {
    ...
    // Camera orbit
    camera.position.x = Math.sin(t * 0.1) * 7;
    camera.position.z = Math.cos(t * 0.1) * 7;
    camera.position.y = Math.sin(t * 0.07) * 2;
    camera.lookAt(0, 0, 0);
